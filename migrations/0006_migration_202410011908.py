...
def forward(old_orm, new_orm):
    ...
    return [
        # Apply default value 30 to the field channel.duration,
        channel.update({channel.duration: 30}).where(channel.duration.is_null(True)),
    ]


def migrate_forward(op, old_orm, new_orm):
    op.add_column(new_orm.channel.duration)
    op.run_data_migration()
    op.add_not_null(new_orm.channel.duration)


def migrate_backward(op, old_orm, new_orm):
    op.run_data_migration()
    op.drop_column(old_orm.channel.duration)
