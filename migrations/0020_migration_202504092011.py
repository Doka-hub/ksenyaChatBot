...
class Notification(peewee.Model):
    ...
    filters = CharField(max_length=255)
...
@snapshot.append
class NotificationButton(peewee.Model):
    notification = snapshot.ForeignKeyField(backref='buttons', index=True, model='notification', on_delete='CASCADE')
    text = CharField(max_length=255)
    url = CharField(max_length=255, null=True)
    class Meta:
        table_name = "notificationbutton"


@snapshot.append
class NotificationImage(peewee.Model):
    notification = snapshot.ForeignKeyField(backref='images', index=True, model='notification', on_delete='CASCADE')
    image = apps.utils.fields.FileField(max_length=255, null=True)
    class Meta:
        table_name = "notificationimage"
...
def forward(old_orm, new_orm):
    ...
    return [
        # Apply default value '' to the field notification.filters,
        notification.update({notification.filters: ''}).where(notification.filters.is_null(True)),
    ]


def migrate_forward(op, old_orm, new_orm):
    op.add_column(new_orm.notification.filters)
    op.run_data_migration()
    op.add_not_null(new_orm.notification.filters)


def migrate_backward(op, old_orm, new_orm):
    op.run_data_migration()
    op.drop_column(old_orm.notification.filters)
