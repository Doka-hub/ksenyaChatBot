...
class Payment(peewee.Model):
    ...
    screenshot = CharField(max_length=255)
...
def forward(old_orm, new_orm):
    payment = new_orm['payment']
    return [
        # Apply default value '' to the field payment.screenshot,
        payment.update({payment.screenshot: ''}).where(payment.screenshot.is_null(True)),
    ]


def migrate_forward(op, old_orm, new_orm):
    op.add_column(new_orm.payment.screenshot)
    op.run_data_migration()
    op.drop_column(old_orm.notification.image)
    op.drop_column(old_orm.notification.file)
    op.add_not_null(new_orm.payment.screenshot)


def migrate_backward(op, old_orm, new_orm):
    op.add_column(new_orm.notification.image)
    op.add_column(new_orm.notification.file)
    op.run_data_migration()
    op.drop_column(old_orm.payment.screenshot)
