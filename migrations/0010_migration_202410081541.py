...
def forward(old_orm, new_orm):
    tguser = new_orm['tguser']
    return [
        # Apply default value False to the field tguser.policy_confirmed,
        tguser.update({tguser.policy_confirmed: False}).where(tguser.policy_confirmed.is_null(True)),
    ]


def migrate_forward(op, old_orm, new_orm):
    op.add_column(new_orm.tguser.policy_confirmed)
    op.run_data_migration()
    op.add_not_null(new_orm.tguser.policy_confirmed)


def migrate_backward(op, old_orm, new_orm):
    op.run_data_migration()
    op.drop_column(old_orm.tguser.policy_confirmed)
