...
def forward(old_orm, new_orm):
    channel = new_orm['channel']
    rbdetails = new_orm['rbdetails']
    return [
        # Apply default value '' to the field channel.telegram_chat_id,
        channel.update({channel.telegram_chat_id: ''}).where(channel.telegram_chat_id.is_null(True)),
        # Apply default value 'text' to the field rbdetails.text,
        rbdetails.update({rbdetails.text: 'text'}).where(rbdetails.text.is_null(True)),
    ]


def migrate_forward(op, old_orm, new_orm):
    op.add_column(new_orm.channel.telegram_chat_id)
    op.add_column(new_orm.rbdetails.text)
    op.run_data_migration()
    op.add_not_null(new_orm.channel.telegram_chat_id)
    op.drop_column(old_orm.rbdetails.field_1)
    op.drop_column(old_orm.rbdetails.account_number)
    op.drop_column(old_orm.rbdetails.field_2)
    op.add_not_null(new_orm.rbdetails.text)


def backward(old_orm, new_orm):
    rbdetails = new_orm['rbdetails']
    return [
        # Apply default value '' to the field rbdetails.field_1,
        rbdetails.update({rbdetails.field_1: ''}).where(rbdetails.field_1.is_null(True)),
        # Apply default value '' to the field rbdetails.account_number,
        rbdetails.update({rbdetails.account_number: ''}).where(rbdetails.account_number.is_null(True)),
        # Apply default value '' to the field rbdetails.field_2,
        rbdetails.update({rbdetails.field_2: ''}).where(rbdetails.field_2.is_null(True)),
    ]


def migrate_backward(op, old_orm, new_orm):
    op.add_column(new_orm.rbdetails.field_1)
    op.add_column(new_orm.rbdetails.account_number)
    op.add_column(new_orm.rbdetails.field_2)
    op.run_data_migration()
    op.drop_column(old_orm.channel.telegram_chat_id)
    op.drop_column(old_orm.rbdetails.text)
    op.add_not_null(new_orm.rbdetails.field_1)
    op.add_not_null(new_orm.rbdetails.account_number)
    op.add_not_null(new_orm.rbdetails.field_2)
