...
@snapshot.append
class StartMessageButton(peewee.Model):
    start_message = snapshot.ForeignKeyField(backref='buttons', index=True, model='startmessage', on_delete='CASCADE')
    button = snapshot.ForeignKeyField(backref='start_messages', index=True, model='buttonmessage', on_delete='CASCADE')
    class Meta:
        table_name = "startmessagebutton"
