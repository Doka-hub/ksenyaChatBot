...
def migrate_forward(op, old_orm, new_orm):
    ...
    op.drop_not_null(new_orm.startmessage.video)
    op.drop_not_null(new_orm.startmessage.photo)


def backward(old_orm, new_orm):
    startmessage = new_orm['startmessage']
    return [
        # Apply default value '' to the field startmessage.video,
        startmessage.update({startmessage.video: ''}).where(startmessage.video.is_null(True)),
        # Apply default value '' to the field startmessage.photo,
        startmessage.update({startmessage.photo: ''}).where(startmessage.photo.is_null(True)),
    ]


def migrate_backward(op, old_orm, new_orm):
    op.run_data_migration()
    op.add_not_null(new_orm.startmessage.video)
    op.add_not_null(new_orm.startmessage.photo)
