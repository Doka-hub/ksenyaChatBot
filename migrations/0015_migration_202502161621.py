...
def forward(old_orm, new_orm):
    buttonmessage = new_orm['buttonmessage']
    return [
        # Check the field `buttonmessage.message` does not contain null values,
    ]


def migrate_forward(op, old_orm, new_orm):
    op.add_column(new_orm.buttonmessage.message)
    op.run_data_migration()
    op.add_not_null(new_orm.buttonmessage.message)
    op.add_foreign_key_constraint(new_orm.buttonmessage.message)
    op.drop_table(old_orm.startmessagebutton)


def migrate_backward(op, old_orm, new_orm):
    op.create_table(new_orm.startmessagebutton)
    op.drop_foreign_key_constraint(old_orm.buttonmessage.message)
    op.run_data_migration()
    op.drop_column(old_orm.buttonmessage.message)
