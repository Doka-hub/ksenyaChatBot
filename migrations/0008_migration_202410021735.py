...
def migrate_forward(op, old_orm, new_orm):
    op.add_column(new_orm.payment.channel)
    op.run_data_migration()
    op.add_foreign_key_constraint(new_orm.payment.channel)


def migrate_backward(op, old_orm, new_orm):
    op.drop_foreign_key_constraint(old_orm.payment.channel)
    op.run_data_migration()
    op.drop_column(old_orm.payment.channel)
